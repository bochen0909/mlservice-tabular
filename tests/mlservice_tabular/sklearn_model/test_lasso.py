...
from mlservice_tabular.sklearn_model import LassoModel
...
@pytest.fixture
def lasso_model():
    return LassoModel(params={"hyperparameters": {"alpha": 1.0}, "columns": {"target": "target"}})
...
def test_model_endpoints_decorator():
    assert hasattr(LassoModel, 'router'), "Decorator should add router attribute to model class"
    assert LassoModel.router.prefix == "/model/sklearn/lasso", "Router should have correct prefix"

# Model functionality tests
def test_model_initialization():
    model = LassoModel(params={"hyperparameters": {"alpha": 2.0}})
...
def test_train(lasso_model, sample_data, tmp_path):
    os.environ['ML_HOME'] = str(tmp_path)
    
    train_path = tmp_path / "train.csv"
    sample_data.to_csv(train_path, index=False)
    
    metadata = lasso_model.train(str(train_path))
    
    assert hasattr(lasso_model.model, 'coef_'), "Model should be fitted"
    assert 'timestamp' in metadata
    assert 'metrics' in metadata
    assert metadata['train_path'] == str(train_path)

def test_predict(lasso_model, sample_data, tmp_path):
    os.environ['ML_HOME'] = str(tmp_path)
    
    # Train the model first
    train_path = tmp_path / "train.csv"
    sample_data.to_csv(train_path, index=False)
    lasso_model.train(str(train_path))
    ...
    prediction_file_path = lasso_model.predict(str(predict_path))
    ...
    assert lasso_model.prediction_column in prediction.columns
    assert len(prediction) == len(predict_data)

def test_load_model(lasso_model, sample_data, tmp_path):
    os.environ['ML_HOME'] = str(tmp_path)
    ...
    sample_data.to_csv(train_path, index=False)
    metadata = lasso_model.train(str(train_path))
    assert hasattr(lasso_model.model, 'coef_')
    ...
    assert isinstance(loaded_model, LassoModel)
...
def test_feature_column_inference(lasso_model, sample_data):
    # Test that feature columns are correctly inferred
    features = lasso_model._infer_features_columns(sample_data.columns)
    assert 'feature1' in features
    assert 'feature2' in features
    assert 'target' not in features

    # Test inference with no columns
    features = lasso_model._infer_features_columns(None)
    assert features == []

def test_target_column_handling(lasso_model, sample_data, tmp_path):
    os.environ['ML_HOME'] = str(tmp_path)
