"""Tests for TabPFN Classification model."""
import os
import json
import pandas as pd
import numpy as np
...
from mlservice_tabular.dl import TabPFNClassificationModel
...
@pytest.fixture
def tabpfn_model():
    return TabPFNClassificationModel(params={"hyperparameters": {}, "columns": {"target": "target"}})
...
def test_model_initialization():
    model = TabPFNClassificationModel(params={"hyperparameters": {}})
    assert isinstance(model.model, TabPFNClassifier)
    
    # Test default parameters
    model = TabPFNClassificationModel()
    assert isinstance(model.model, TabPFNClassifier)

def test_train(tabpfn_model, sample_classification_data, tmp_path):
    os.environ['ML_HOME'] = str(tmp_path)
    
    train_path = tmp_path / "train.csv"
    sample_classification_data.to_csv(train_path, index=False)
    
    metadata = tabpfn_model.train(str(train_path))
    
    assert isinstance(tabpfn_model.model, TabPFNClassifier), "Model should be fitted"
...
def test_predict(tabpfn_model, sample_classification_data, tmp_path):
    os.environ['ML_HOME'] = str(tmp_path)
    
    # Train the model first
    train_path = tmp_path / "train.csv"
    sample_classification_data.to_csv(train_path, index=False)
    tabpfn_model.train(str(train_path))
    ...
    prediction_file_path = tabpfn_model.predict(str(predict_path))
    ...
    assert tabpfn_model.prediction_column in prediction.columns
    assert tabpfn_model.predict_proba_column in prediction.columns
    assert len(prediction) == len(predict_data)
    assert ((prediction[tabpfn_model.prediction_column] == 0) | 
            (prediction[tabpfn_model.prediction_column] == 1)).all()
    assert ((prediction[tabpfn_model.predict_proba_column] >= 0) & 
            (prediction[tabpfn_model.predict_proba_column] <= 1)).all()

def test_load_model(tabpfn_model, sample_classification_data, tmp_path):
    os.environ['ML_HOME'] = str(tmp_path)
    ...
    sample_classification_data.to_csv(train_path, index=False)
    metadata = tabpfn_model.train(str(train_path))
    assert isinstance(tabpfn_model.model, TabPFNClassifier)
    ...
    assert isinstance(loaded_model.model, TabPFNClassifier)
    assert hasattr(loaded_model.__class__, 'router')
    assert isinstance(loaded_model, TabPFNClassificationModel)
...
def test_model_endpoints(client, sample_classification_data, tmp_path):
    ...
    response = client.post(
        "/model/dl/tabpfn_classification/train",
        json={
    ...
    response = client.post(
        "/model/dl/tabpfn_classification/predict",
        json={
