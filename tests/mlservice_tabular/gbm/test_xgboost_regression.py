...
from mlservice_tabular.gbm import XGBoostRegressionModel
...
@pytest.fixture
def xgboost_model():
    return XGBoostRegressionModel(params={"hyperparameters": {"n_estimators": 100}, "columns": {"target": "target"}})
...
def test_model_endpoints_decorator():
    assert hasattr(XGBoostRegressionModel, 'router'), "Decorator should add router attribute to model class"
    assert XGBoostRegressionModel.router.prefix == "/model/gbm/xgboost", "Router should have correct prefix"

# Model functionality tests
def test_model_initialization():
    model = XGBoostRegressionModel(params={"hyperparameters": {"n_estimators": 50}})
...
def test_predict(xgboost_model, sample_data, tmp_path):
    os.environ['ML_HOME'] = str(tmp_path)
...
def test_load_model(xgboost_model, sample_data, tmp_path):
    os.environ['ML_HOME'] = str(tmp_path)
...
def test_feature_column_inference(xgboost_model, sample_data):
    # Test that feature columns are correctly inferred
    features = xgboost_model._infer_features_columns(sample_data.columns)
    assert 'feature1' in features
    assert 'feature2' in features
    assert 'target' not in features

    # Test inference with no columns
    features = xgboost_model._infer_features_columns(None)
    assert features == []

def test_target_column_handling(xgboost_model, sample_data, tmp_path):
    os.environ['ML_HOME'] = str(tmp_path)
