"""Tests for LightGBM Classification model."""
import os
import json
import pandas as pd
import numpy as np
...
from mlservice_tabular.gbm import LightGBMClassificationModel
...
@pytest.fixture
def lightgbm_model():
    return LightGBMClassificationModel(params={"hyperparameters": {"n_estimators": 100}, "columns": {"target": "target"}})
...
def test_model_initialization():
    model = LightGBMClassificationModel(params={"hyperparameters": {"n_estimators": 50}})
    assert model.hyperparameters["n_estimators"] == 50
    
    # Test default parameters
    model = LightGBMClassificationModel()
    assert isinstance(model.model, lgb.LGBMClassifier)
    assert model.hyperparameters['objective'] == 'binary'

def test_train(lightgbm_model, sample_classification_data, tmp_path):
...
def test_predict(lightgbm_model, sample_classification_data, tmp_path):
    os.environ['ML_HOME'] = str(tmp_path)
    ...
    assert lightgbm_model.predict_proba_column in prediction.columns
    assert len(prediction) == len(predict_data)
    assert ((prediction[lightgbm_model.prediction_column] == 0) | 
            (prediction[lightgbm_model.prediction_column] == 1)).all()
    assert ((prediction[lightgbm_model.predict_proba_column] >= 0) & 
            (prediction[lightgbm_model.predict_proba_column] <= 1)).all()

def test_load_model(lightgbm_model, sample_classification_data, tmp_path):
    os.environ['ML_HOME'] = str(tmp_path)
    ...
    assert isinstance(loaded_model, LightGBMClassificationModel)
...
def test_model_endpoints(client, sample_classification_data, tmp_path):
    ...
    response = client.post(
        "/model/gbm/lightgbm_classification/train",
        json={
    ...
    response = client.post(
        "/model/gbm/lightgbm_classification/predict",
        json={
